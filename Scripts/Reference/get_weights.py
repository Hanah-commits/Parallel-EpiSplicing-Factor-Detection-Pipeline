...
logFC_Scores = pd.read_csv('logFC.csv', delimiter='\t', header=None, names=['Gene stable ID', 'logFC'])
names = pd.read_csv('GeneID_Name.csv', delimiter='\t')
...
rbp = ['A1CF', 'ANKHD1', 'CELF4', 'CELF5', 'CELF6', 'CNOT4', 'CPEB2', 'CPEB4', 'DAZAP1', 'ENOX1', 'ESRP2', 'FMR1',
       ...
       'MBNL1', 'MSI1', 'PABPC1', 'PABPC3', 'PABPC4', 'PABPC5', 'PABPN1', 'PCBP1', 'PCBP2', 'PPRC1', 'PTBP1', 'PUM2',
...
logFC_Scores['Gene stable ID'] = logFC_Scores['Gene stable ID'].str.split('.').str[0]
...
logFC_Scores['Gene name'] = logFC_Scores['Gene name'].replace({'CELF4': 'BRUNOL4',
                                                               ...
                                                               'CELF6': 'BRUNOL6',
                                                               'ELAVL1': 'HuR'})
logFC_Scores[['Gene name', 'logFC']].to_csv('rbp_logFC.csv', sep='\t', index=False, header=['rbp', 'logFC'])

# noscore = list(set(names['Gene stable ID'].values) - set(logFC_Scores['Gene stable ID'].values))
# noscore = names[names['Gene stable ID'].isin(noscore)]
# print(noscore['Gene name'].values)
